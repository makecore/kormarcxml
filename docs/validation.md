...
## Application-level presence (2026-09-19)

Level 3 now applies the annex M requirements to Leader/17 blank (full) or 7 (minimal): 001, 003, 005, 008, 040, 245 and 300. Absent conditional A fields produce an applicability-unknown information diagnostic unless applicability is confirmed. Per-record `profile={"application": {"applicable_fields": ["020"]}}` supplies such a fact; it must not be applied indiscriminately to heterogeneous batches. Leader/07 a/b/d requires 773; Leader/17 4 requires 042. Other input levels are not silently mapped to full or minimal. See [source audit and KX-015](application-level-audit.md). Presence is not a content certification; level 2 remains suitable for structural checks on fragments.
